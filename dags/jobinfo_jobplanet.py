        ...
        for company in companies:
            ...
            # 없으면 만듦
                ...
                try:
                    ...
                    search_company = driver.find_element(
                        By.ID, "search_bar_search_query"
                    )
                    ...
                    driver.find_element(
                        By.XPATH,
                        "/html/body/div[1]/div[2]/div/div[1]/div[1]/div/div[2]/div[1]/div/a",
                    ).click()
                    driver.implicitly_wait(wait)
                    logging.info(
                        f"{company} 회사 정보를 알기 위해 상세정보 클릭합니다."
                    )
                    # 크롤링 시행 바로밑에 있음
                    review_data = review_crawling(driver)
                    upload_to_s3(
                        review_data,
                        f"path/in/s3/bucket/review_information_{company}.csv",
                        bucket_name,
                    )
                    file_list.append(
                        f"path/in/s3/bucket/review_information_{company}.csv"
...
def review_crawling(driver):
    ...
    try:
        review_count = (
            driver.find_element(By.ID, "viewReviewsTitle")
    ...
    # 전체 직군 면접 리뷰 갯수 세는거
    interview_count = (
        driver.find_element(By.ID, "viewInterviewsTitle")
        .find_element(By.CLASS_NAME, "num")
        .text
    )
    ...
    try:
        ...
        interview_count = (
            driver.find_element(By.ID, "viewInterviewsTitle")
            .find_elements(By.CLASS_NAME, "num")[1]
            .text
        )
        interview_count = int(interview_count)
    except:
    ...
    for _ in range(page):
        ...
        for i in reviews:
            try:
                ...
                # 문장 한 줄로 바꾸고 전처리
                review_summary = (
                    review_summary.replace(".", "").replace("\n", " ").replace('"', "")
                )
...
def wordcloud_info(company):
    ...
    try:
        obj = s3_client.get_object(
            ...
            Key=f"path/in/s3/bucket/review_information_{company}.csv",
        )
        df = pd.read_csv(obj["Body"], encoding="utf-8")
        logging.info(df.head())
        ...
        review_summary_counter = {
            k: v for k, v in review_summary_counters.items() if len(k) >= word
        }

        # 위에꺼, 갯수가 많은 순서대로 정렬
        if len(review_summary_counter) > 0:
            key = sorted(
                review_summary_counter, key=review_summary_counter.get, reverse=True
            )
        ...
        # 조건에 만족 못했어도 넣어야 하니까, 원본을 한줄로 만듦
            ...
            key = sorted(
                review_summary_counters, key=review_summary_counters.get, reverse=True
            )
            review_summary_key = key[:10]
            review_summary = ", ".join(review_summary_key)
        ...
        interview_summary_counter = {
            k: v for k, v in interview_summary_counters.items() if len(k) >= word
        }
        if len(interview_summary_counter) > 0:
            key = sorted(
                interview_summary_counter,
                key=interview_summary_counter.get,
                reverse=True,
            )
            ...
            key = sorted(
                interview_summary_counters,
                key=interview_summary_counters.get,
                reverse=True,
            )
            interview_summary_key = key[:10]
            interview_summary = ", ".join(interview_summary_key)
        ...
        interview_question_counter = {
            k: v for k, v in interview_question_counters.items() if len(k) >= word
        }
        if len(interview_question_counter) > 0:
            key = sorted(
                interview_question_counter,
                key=interview_question_counter.get,
                reverse=True,
            )
            ...
            key = sorted(
                interview_question_counters,
                key=interview_question_counters.get,
                reverse=True,
            )
            interview_question_key = key[:10]
            interview_question = ", ".join(interview_question_key)
        ...
        interview_answer_counter = {
            k: v for k, v in interview_answer_counters.items() if len(k) >= word
        }
        if len(interview_answer_counter) > 0:
            key = sorted(
                interview_answer_counter, key=interview_answer_counter.get, reverse=True
            )
            ...
            key = sorted(
                interview_answer_counters,
                key=interview_answer_counters.get,
                reverse=True,
            )
            interview_answer_key = key[:10]
            interview_answer = ", ".join(interview_answer_key)
...
def upload_to_s3(dataframe, file_name, bucket_name):
    ...
    try:
        ...
        s3_clinet.put_object(
            Body=csv_buffer.getvalue(), Bucket=bucket_name, Key=file_name
        )
